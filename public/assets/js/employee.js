...
function renderEmployeeTable() {
  ...
  if (!rows) {
    rows = renderEmptyTableRow(
      $("#employeeTable").closest("table").find("thead th").length,
    );
  }
  ...
  const startItem =
    pagination.totalItems === 0
      ? 0
      : (pagination.currentPage - 1) * employeePerPage + 1;
  const endItem = Math.min(
    pagination.currentPage * employeePerPage,
...
function deleteSelectedEmployees() {
  ...
  swalConfirm(
    `Delete ${ids.length} selected employee${ids.length === 1 ? "" : "s"}?`,
    function () {
...
$(function () {
  ...
  $("#filterDepartment, #filterGender, #filterStatus").on(
    "change",
    loadEmployees,
  );
