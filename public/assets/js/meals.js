...
function updateMealHeadcountSummary(meals, label, range) {
  const totalHeadcount = (meals || []).reduce(
    (sum, meal) => sum + Number(meal?.headcount ?? meal?.active_count ?? 0),
    0,
  );
  const totalCompanyPay = (meals || []).reduce(
    (sum, meal) => sum + Number(meal?.company_pay ?? meal?.headcount ?? 0),
    0,
  );
  const totalLunchBox = (meals || []).reduce(
    (sum, meal) => sum + Number(meal?.lunch_box ?? meal?.meal_count ?? 0),
    0,
  );
  const totalArrivals = (meals || []).reduce(
    (sum, meal) => sum + (meal?.arrivals?.length || 0),
    0,
  );
  const totalDepartures = (meals || []).reduce(
    (sum, meal) => sum + (meal?.departures?.length || 0),
    0,
  );
  const averageDailyHeadcount = meals.length
    ? Math.round(totalHeadcount / meals.length)
    : 0;
  ...
  return `
        <div class="meal-planner-people meal-planner-people--${type}">
            ${people
              .map((person) => {
                const name =
                  person.english_name || person.employee_code || "Employee";
...
function renderMealPlannerRow(meal) {
  ...
  const dayLabel = parseLocalDate(meal.date).toLocaleDateString("en-US", {
    weekday: "long",
  });
...
function renderMealPlannerTable(meals) {
  ...
  if (!meals || !meals.length) {
    $("#mealTable").html(
      '<tr><td colspan="8" class="text-center text-muted">No meal data available.</td></tr>',
    );
    return;
  }
...
function loadMealsByWeek() {
  ...
  $.get(
    mealApiUrl(
      `api/meals.php/range?start_date=${range.startDate}&end_date=${range.endDate}`,
    ),
    function (data) {
