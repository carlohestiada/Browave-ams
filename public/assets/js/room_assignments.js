...
const assignmentSortColumns = [
  {
    index: 1,
    key: function (row) {
      return `${row.employee_code || ""} ${row.english_name || ""}`;
    },
  },
...
function renderAssignEmployeeDropdown() {
  ...
  if ($.fn.select2) {
    if (
      assignSelect.hasClass("select2-hidden-accessible") ||
      assignSelect.data("select2")
    ) {
      try {
...
function initializeAssignmentDateBounds() {
  ...
  if (!checkout.value || !assignmentCheckoutManuallyEdited) {
    checkout.value =
      calculateCheckoutDate(checkin.value || today) || checkin.value || today;
  }
...
$(function () {
  ...
  $("#assignForm").on("submit", function (e) {
    ...
    if (activeAssignedEmployees.has(selectedEmployee)) {
      swalError(
        "This employee already has an active room assignment. Use transfer instead.",
      );
      return;
    }
    ...
    $.post(raApi, $(this).serialize(), function (resp) {
      ...
      if (res.success) {
        ...
        const assignModal =
          bootstrap.Modal.getInstance(assignEl) ||
          new bootstrap.Modal(assignEl);
        assignModal.hide();
        resetAssignForm();
        loadAssignments();
      } else {
  ...
  $("#transferForm").on("submit", function (e) {
    ...
    $.ajax({
      ...
      error: function (xhr) {
        const msg =
          xhr.responseJSON?.error || xhr.responseText || "Transfer failed";
        swalError(msg);
      },
    });
  });
...
function openTransfer(id = null, lockAssignment = false) {
  ...
  const transferEl = document.getElementById("transferModal");
  const transferModal =
    bootstrap.Modal.getInstance(transferEl) || new bootstrap.Modal(transferEl);
  transferModal.show();
...
function deleteAssignment(id) {
  swalConfirm("Delete this room assignment?", function () {
    $.ajax({
      ...
      error: function (xhr) {
        swalError(
          xhr.responseJSON?.error || xhr.responseText || "Delete failed",
        );
      },
    });
  });
...
function deleteSelectedAssignments() {
  ...
  swalConfirm(
    `Delete ${ids.length} selected room assignment${ids.length === 1 ? "" : "s"}?`,
    function () {
      ...
      Promise.all(ids.map(deleteAssignmentById)).then((results) => {
        ...
        if (failed.length > 0) {
          ...
          swalError(
            `${deletedCount} deleted. ${failed.length} could not be deleted. ${firstError}`,
        ...
        swalSuccess(
          `${deletedCount} room assignment${deletedCount === 1 ? "" : "s"} deleted successfully.`,
        );
      });
    },
  );
